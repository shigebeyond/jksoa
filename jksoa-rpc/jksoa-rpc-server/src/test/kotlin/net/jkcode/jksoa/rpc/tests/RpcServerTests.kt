...
class RpcServerTests {
    ...
    fun testUrl(){
        ...
        val url = Url(config["rpc-protocol"]!!, config.getString("host", getIntranetHost())!!, config["port"]!!, "net.jkcode.jksoa.rpc.example.IExampleService", config.getMap("parameters", emptyMap<String, Any?>())!!)
        println(url)
    }
